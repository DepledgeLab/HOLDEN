...
library(dplyr)
A_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "A")
m6A_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("a")& primary_base == "A")
...
A_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "A")
m6A_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("a")& primary_base == "A")
...
write.table(A_merged_df_9, "HD10.6_DMSO_STM_A_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
write.table(m6A_merged_df_9, "HD10.6_DMSO_STM_m6A_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
...
pdf(file = "A) HD10.6,A&m6A all-context in DMSO&STM in hg38&IVT,0.9.0,Profile Plot.pdf", width = 20, height = 5)
combined_plot_9
dev.off()
...
#Generating the merged dataframe with ranges and fractions
library(dplyr)
A_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.m6A_DRACH.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-"))
m6A_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.m6A_DRACH.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("a"))
A_IVT_filtered_9 <- NHDF_polyA_TSO_IVT.sup.m6A_DRACH.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-"))
m6A_IVT_filtered_9 <- NHDF_polyA_TSO_IVT.sup.m6A_DRACH.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("a"))
A_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.m6A_DRACH.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-"))
m6A_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.m6A_DRACH.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("a"))
...
write.table(A_merged_df_9, "HD10.6_DMSO_STM_A_DRACH_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
write.table(m6A_merged_df_9, "HD10.6_DMSO_STM_m6A_DRACH_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
...
pdf(file = "B) HD10.6,A&m6A(DRACH)in DMSO&STM in hg38&IVT,0.9.0,Profile Plot.pdf", width = 20, height = 5)
combined_plot_9
dev.off()
...
#Generating the merged dataframe with ranges and fractions
library(dplyr)
U_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "T")
pseU_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("17802")& primary_base == "T")
U_IVT_filtered_9_DMSO <- NHDF_polyA_TSO_IVT.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "T")
pseU_IVT_filtered_9_DMSO <- NHDF_polyA_TSO_IVT.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("17802")& primary_base == "T")
U_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "T")
pseU_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("17802")& primary_base == "T")


U_merged_df_9 <- U_hg38_filtered_9_DMSO %>%
  inner_join(U_hg38_filtered_9_STM, by = c("range_start", "range_end")) %>%
  inner_join(U_IVT_filtered_9_DMSO, by = c("range_start", "range_end")) 

pseU_merged_df_9 <- pseU_hg38_filtered_9_DMSO %>%
  inner_join(pseU_hg38_filtered_9_STM, by = c("range_start", "range_end")) %>%
...
write.table(U_merged_df_9, "HD10.6_DMSO_STM_U_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
write.table(pseU_merged_df_9, "HD10.6_DMSO_STM_pseU_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
...
pdf(file = "C) HD10.6,U&pseU in DMSO&STM in hg38&IVT,0.9.0,Profile Plot.pdf", width = 20, height = 5)
combined_plot_9
dev.off()
...
library(dplyr)
A_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "A")
i_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("17596")& primary_base == "A")
A_IVT_filtered_9_DMSO <- NHDF_polyA_TSO_IVT.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "A")
i_IVT_filtered_9_DMSO <- NHDF_polyA_TSO_IVT.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("17596")& primary_base == "A")
A_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "A")
i_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("17596")& primary_base == "A")

A_merged_df_9 <- A_hg38_filtered_9_DMSO %>%
  inner_join(A_hg38_filtered_9_STM, by = c("range_start", "range_end")) %>%
  inner_join(A_IVT_filtered_9_DMSO, by = c("range_start", "range_end"))

i_merged_df_9 <- i_hg38_filtered_9_DMSO %>%
  inner_join(i_hg38_filtered_9_STM, by = c("range_start", "range_end")) %>%
...
#write.table(A_merged_df_9, "HD10.6_DMSO_STM_A_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
write.table(i_merged_df_9, "HD10.6_DMSO_STM_inosine_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
...
pdf(file = "D) HD10.6, A&inosine in DMSO&STM in hg38&IVT,0.9.0,Profile Plot.pdf", width = 20, height = 5)
combined_plot_9
dev.off()
...
#Generating the merged dataframe with ranges and fractions
library(dplyr)
C_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "C")
m_hg38_filtered_9_DMSO <- HD10_6_DMSO_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("m")& primary_base == "C")
C_IVT_filtered_9_DMSO <- NHDF_polyA_TSO_IVT.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "C")
m_IVT_filtered_9_DMSO <- NHDF_polyA_TSO_IVT.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("m")& primary_base == "C")
C_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("-")& primary_base == "C")
m_hg38_filtered_9_STM <- HD10_6_STM2457_48h_1.sup.allMods.trimAdapters.dorado.0.9.0_hg38_probabilities %>% filter (code %in% c("m")& primary_base == "C")


C_merged_df_9 <- C_hg38_filtered_9_DMSO %>%
  inner_join(C_hg38_filtered_9_STM, by = c("range_start", "range_end")) %>%
  inner_join(C_IVT_filtered_9_DMSO, by = c("range_start", "range_end"))

m_merged_df_9 <- m_hg38_filtered_9_DMSO %>%
  inner_join(m_hg38_filtered_9_STM, by = c("range_start", "range_end")) %>%
...
write.table(C_merged_df_9, "HD10.6_DMSO_STM_C_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
write.table(m_merged_df_9, "HD10.6_DMSO_STM_m_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
...
pdf(file = "E) HD10.6, C&m5C in DMSO&STM in hg38&IVT,0.9.0,Profile Plot.pdf", width = 20, height = 5)
combined_plot_9
dev.off()
