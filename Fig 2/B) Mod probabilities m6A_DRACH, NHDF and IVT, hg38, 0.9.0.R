...
write.table(A_merged_df_9, "NHDF_DMSO_STM_A_DRACH_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
write.table(m6A_merged_df_9, "NHDF_DMSO_STM_m6A_DRACH_hg38&IVT_0.9.0_merged_df.txt", sep = "\t", row.names = FALSE)
...
pdf(file = "B) m6A(DRACH)in DMSO&STM in hg38&IVT,0.9.0,Profile Plot.pdf", width = 14.5, height = 5)
combined_plot_9
dev.off()


###Into the supplementary
pdf(file = "C) A(DRACH) in DMSO&STM in hg38&IVT,0.9.0,Profile Plot.pdf", width = 7.5, height = 5)
A_9
dev.off()
